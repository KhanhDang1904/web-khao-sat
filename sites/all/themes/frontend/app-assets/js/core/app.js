...
(function (window, document, $) {
  ...
  window.getToastError = function getToastError(r1){
    if(r1.status == 404)
      ...
      var $message = r1.responseText.replace('&lt;div&gt;', '<div>').replace('&lt;/div&gt;', '</div>');
      let parser = new DOMParser();
      const doc = parser.parseFromString($message, 'text/html');
      if(doc.getElementsByTagName('div').length > 0){
        toastr['error'](doc.getElementsByTagName('div')[0].innerText, 'Thông báo!', {
          closeButton: true,
          tapToDismiss: false,
          rtl: false
        });
      }else{
        toastr['error'](r1.responseText, 'Thông báo!', {
          closeButton: true,
          tapToDismiss: false,
          rtl: false
        });
  ...
  window.getToastSuccess = function getToastSuccess($content){
  ...
  window.toastError = function toastErrer($content){
  ...
  window.blockForm = function blockForm(formSection){
    formSection.block({
      message: '<div class="spinner-border text-primary" role="status"></div>',
      timeout: 1000,
      css: {
  ...
  window.loadData = function loadData($url, $data, callbackBefore, callbackSuccess){
    $.ajax({
      url: $url,
      data: $data,
      dataType: 'json',
