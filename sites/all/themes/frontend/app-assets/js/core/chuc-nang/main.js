$(window).on('load', function () {
  ...
  if ($(".flatpickr-customs").length > 0) {
    $('.flatpickr-customs').each(function () {
      $(this).flatpickr({
        static: true,
        dateFormat:"m/d/Y"
      })
  ...
  window.getTableModal= function  getTableModal($url,$data,$table,$columns){
    loadData($url,$data,
      function(){
        blockForm($($table))
      },function(data){
        loadDataTable($table,data.data,$columns,data.perPage,data.countPage)
        loadIcon()
      })
    $(document).on("click", $table+' table tfoot .pagination li a', function (e) {
      var  $dataPerPage = $data;
      $dataPerPage.perPage=$(this).attr("data-value");
      $dataPerPage.limit=$($table+" table tfoot #table_limit").val();
      loadData($url,$dataPerPage,
        function(){
          blockForm($($table))
        },function(data){
          loadDataTable($table,data.data,$columns,data.perPage,data.countPage)
    ...
    $(document).on("change", $table+' table tfoot #table_limit', function (e) {
      var  $dataLimit= $data;
      $dataLimit.perPage=1;
      $dataLimit.limit=$(this).val();
      loadData($url,$dataLimit,
        function(){
          blockForm($($table))
        },function(data){
          loadDataTable("#table-danh-sach-nhu-cau",data.data,$columns,data.perPage,data.countPage)
          loadIcon()
        })
    })
  }
  ...
  $(document).on("click",".main-menu-content .dropdown-toggle",function(e){
    e.preventDefault();
...
function loadDataForm2($data,$form){
  $.each($data, function (key,value){
    $($form+' #'+key).html(value)
  })
}
function loadDataValue($data,$form){
  $.each($data, function (key,value){
    $($form+' #'+key).val(value)
  })
}
    ...
    $('.select2-data-ajax').each(function () {
      ...
      if (!$this.hasClass('select2-done')) {
        $this.select2({
          ...
          placeholder: 'Họ tên hoặc điện thoại',
          escapeMarkup: function (markup) {
            return markup;
          }, // let our custom formatter work
          minimumInputLength: 2,
...
function formatRepoSelection(repo) {
  $("#field_khach_hang_form").val(repo.name)
  return repo.name || repo.text;
}
function formatRepo(repo) {
  if (repo.loading) {
